...
char **assign_string( char **string_array, char *string, int index )
{
	string_array[ index ] = malloc ( strlen( string ) + 1 );
	string_array[ index ] = strcpy( string_array[ index ], string );
	return string_array;
}

void print_string_array( char **string_array, int size )
{
	int i;
	for( i = 0; i < size; i++ )
...
void release_string_array( char **string_array, int size )
{
	int i;
	for( i = 0; i < size-1; i++ )
		free( string_array[ i ] );
...
char *pcharlist2string( unsigned long term, int size )
{
	char *expression;
	char *buf = malloc( size * sizeof( char ) );
	
	expression = p2c_chars(term, buf, size );
	
	return expression;
}

int *intpointer( int num )
{
	return &num;
}
